...
def run_comparison_f32():
  """Compare NumPy (double) and JAX (single) versions of the Forward Curvelet Transform."""
  print("--- Fast Curvelet Transform: NumPy (double) vs JAX (single) Comparison ---")
  ...
  # 2. Forward Transform: NumPy (Double Precision)
  ...
  # 3. Forward Transform: JAX (Single Precision)
  ...
  # 4. Compare Coefficients (comparing Single vs Double)
  print("\nComparing coefficients (JAX Single vs NP Double)...")
  max_diff_coeffs = 0
  ...
  print("\n[NumPy Double] Computing inverse transform from NumPy coefficients...")
  rec_np = ifdct_wrapping(
    coeffs_np, is_real=is_real, m_img=n1, n_img=n2
  )
  
  print("[NumPy Double] Computing inverse transform from JAX coefficients...")
  coeffs_jax_np = [[np.array(c).astype(np.complex128) for c in scale] for scale in coeffs_jax]
  ...
  axes[1].set_title("Reconstructed (from JAX Single)")
  axes[1].axis('off')
  
  diff_map = np.abs(np.real(rec_np) - np.real(rec_jax))
  im = axes[2].imshow(diff_map, cmap='hot')
  axes[2].set_title("Abs Difference (NP Double vs JAX Single)")
  axes[2].axis('off')
  plt.colorbar(im, ax=axes[2])
  
  plt.tight_layout()
  plt.savefig('np_jax_comparison_f32.png')
  print("\nArtifact saved: np_jax_comparison_f32.png")
  plt.show()
