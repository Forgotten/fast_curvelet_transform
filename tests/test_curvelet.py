...
class TestCurvelet(parameterized.TestCase):
  ...
  def test_window_properties(self):
    ...
    # Check boundaries.
    self.assertTrue(np.isclose(wl[0], 0, atol=1e-12))
    self.assertTrue(np.isclose(wr[0], 1, atol=1e-12))
    self.assertTrue(np.isclose(wl[-1], 1, atol=1e-12))
    self.assertTrue(np.isclose(wr[-1], 0, atol=1e-11))

  @parameterized.product(
    shape=[(32, 32), (64, 64), (64, 32)],
    is_real=[True, False],
    finest=["wavelets", "curvelets"]
  )
  def test_identity(self, shape, is_real, finest):
    ...
    self.assertEqual(x_rec.dtype, dtype)
  ...
  def test_energy_preservation(self, shape, is_real, finest):
  ...
  def test_scale_orthogonality(self, is_real):
    ...
    for j in range(len(reconstructions)):
      for k in range(j + 2, len(reconstructions)):
          ...
          if norm_j > 1e-12 and norm_k > 1e-12:
              self.assertLess(inner / (norm_j * norm_k), 1e-10)
  ...
  def test_adjoint(self, shape, is_real, finest):
